...
bool RayTracer::RenderDefault(const DefaultRenderingProperties &properties) {
  if (properties.m_frameSize.x == 0 | properties.m_frameSize.y == 0)
    return true;
  ...
  if (properties.m_frameSize !=
      m_defaultRenderingLaunchParams.m_defaultRenderingProperties.m_frameSize) {
...
#pragma region Copy results to output texture
  ...
  inputLayer[0].rowStrideInBytes =
      m_defaultRenderingLaunchParams.m_frame.size.x * sizeof(glm::vec4);
  ...
  case OutputType::DenoisedColor: {
    ...
    CUDA_CHECK(MemcpyToArray(outputArray, 0, 0, (void *)outputLayer.data,
                             sizeof(glm::vec4) *
                                 m_defaultRenderingLaunchParams.m_frame.size.x *
                                 m_defaultRenderingLaunchParams.m_frame.size.y,
                             cudaMemcpyDeviceToDevice));
  } break;
  }
...
#pragma region Bind output texture
          ...
          CUDA_CHECK(GraphicsGLRegisterImage(
              &graphicsResource, instance.m_albedoTexture, GL_TEXTURE_2D,
...
#pragma region Bind output texture
          ...
          cudaGraphicsResource_t graphicsResource;
          CUDA_CHECK(GraphicsGLRegisterImage(
              &graphicsResource, instance.m_normalTexture, GL_TEXTURE_2D,
              cudaGraphicsRegisterFlagsReadOnly));
          CUDA_CHECK(GraphicsMapResources(1, &graphicsResource, nullptr));
          CUDA_CHECK(GraphicsSubResourceGetMappedArray(&textureArray,
                                                       graphicsResource, 0, 0));
